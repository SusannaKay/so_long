...
void load_direction(t_map *map, char *dir, int i)
{
    ...
    map->animation->frames[i][0] = mlx_xpm_file_to_image(map->mlx, path, &size, &size);
    free(path);
    path = ft_strjoin(dir, "1.xpm");
    map->animation->frames[i][1] = mlx_xpm_file_to_image(map->mlx, path, &size, &size);
    free(path);
    path = ft_strjoin(dir, "2.xpm");
    map->animation->frames[i][2] = mlx_xpm_file_to_image(map->mlx, path, &size, &size);
    free(path);
    path = ft_strjoin(dir, "3.xpm");
    map->animation->frames[i][3] = mlx_xpm_file_to_image(map->mlx, path, &size, &size);
    free(path);
}
...
{
    if (map->animation->frame_delay++ >= 10)
    {
